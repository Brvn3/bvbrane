...
if (strpos($a, "Your card's security code is incorrect.")) {
 echo '<span class="badge badge-success">#Aprovada</span> '.$cc.' '.$mes.' '.$ano.' '.$cvv.' <b style="color: white;"> ❤Live❤ '.$bin.'('.$banco.'-'.$nivel.') @brwcv <br>';
  }
else if(substr_count($c, 'incorrect_number') > 0){
  echo '<span class="badge badge-danger">💀Rejected💀</span> '.$cc.' '.$mes.' '.$ano.' '.$cvv.' <b> ❌ Invalid ❌  @brwcv</b>';
  exit();
  }
 ...
 echo '<span class="badge badge-success">#Aprovada</span> '.$cc.' '.$mes.' '.$ano.' '.$cvv.' <b style="color: white;"> ❤ Live ❤ '.$bin.'('.$banco.'-'.$nivel.') @brwcv <br>';
  ...
  echo '<span class="badge badge-danger">Rejected</span> '.$cc.' '.$mes.' '.$ano.' '.$cvv.' <b>🔴 Blocked 🔴'.$bin.'() @brwcv </b>';
}


else if (strpos($a, "Your card was declined.")) {
  echo '<span class="badge badge-danger">Rejected</span> '.$cc.' '.$mes.' '.$ano.' '.$cvv.' <b>🔴 Dead 🔴'.$bin.'() @brwcv </b>';
}
 ...
 echo '<span class="badge badge-danger">Rejected</span> '.$cc.' '.$mes.' '.$ano.' '.$cvv.' <b>🔴 Unknown 🔴 '.$bin.'() @brwcv </b>';
}
